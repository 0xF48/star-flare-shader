...
float flare(in float angle,in float alpha,in float time){
	...
	float height = 3.0;
	float height_noise = n*6.0;


	float g = pow(pow(1.0+sin((angle+time*20.3)+noise(time+angle+alpha)*(20.0*alpha))*(1.4*n),height_noise)*alpha,height_noise);
	

	g *= pow(((angle+PI) * (TWO_PI - angle - PI)),1.2)*0.01; //fade out at pole

	// g *= pow(((angle-PI) * (TWO_PI + angle + PI)),1.5)*0.11;
	g *= alpha * alpha;
	return alpha + g;
}
...
void main() {
	...
	if(alpha > 0.9){
		alpha = (1.0 - light(uv,SIZE,RADIUS*0.01,INNER_FADE*0.65,OUTER_FADE))*3.5;
		f = flare(angle,alpha*0.5,iTime*0.3);
	}else if(alpha < 0.01){
		f = alpha;
	}else{
